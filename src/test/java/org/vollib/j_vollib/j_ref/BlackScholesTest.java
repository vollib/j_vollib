...
import org.vollib.j_vollib.j_ref.black_scholes.ImpliedVolatility;
import org.vollib.j_vollib.j_ref.black_scholes.greeks.Analytical;
import org.vollib.j_vollib.j_ref.black_scholes.greeks.Numerical;
import org.vollib.j_vollib.j_ref.black_scholes.BlackScholes;
...
public class BlackScholesTest {
    ...
    public void test_prices() {
        for (int i = 0; i < SIZE; i++) {
            ...
            double actual_call = BlackScholes.black_scholes('c', S, K, t, r, sigma);
            assertEquals("Error at " + i, expected_call, actual_call, EPSILON);

            double expected_put = testData.get("bs_put").get(i);
            double actual_put = BlackScholes.black_scholes('p', S, K, t, r, sigma);
    ...
    public void test_analytical_delta() {
        for (int i = 0; i < SIZE; i++) {
            ...
            double actual_call = Analytical.delta('c', S, K, t, r, sigma);
            assertEquals("Error at " + i, expected_call, actual_call, EPSILON);

            double expected_put = testData.get("PD").get(i);
            double actual_put = Analytical.delta('p', S, K, t, r, sigma);
            assertEquals("Error at " + i, expected_put, actual_put, EPSILON);
        }
    }
    ...
    public void test_analytical_theta() {
        for (int i = 0; i < SIZE; i++) {
            ...
            double actual_call = Analytical.theta('c', S, K, t, r, sigma) * 365.0;
            assertEquals("Error at " + i, expected_call, actual_call, EPSILON);

            double expected_put = testData.get("PT").get(i);
            double actual_put = Analytical.theta('p', S, K, t, r, sigma) * 365.0;
            assertEquals("Error at " + i, expected_put, actual_put, EPSILON);
        }
    }
    ...
    public void test_analytical_gamma() {
        for (int i = 0; i < SIZE; i++) {
            ...
            double actual_call = Analytical.gamma('c', S, K, t, r, sigma);
            assertEquals("Error at " + i, expected_call, actual_call, EPSILON);

            double expected_put = testData.get("PG").get(i);
            double actual_put = Analytical.gamma('p', S, K, t, r, sigma);
            assertEquals("Error at " + i, expected_put, actual_put, EPSILON);
        }
    }
    ...
    public void test_analytical_vega() {
        for (int i = 0; i < SIZE; i++) {
            ...
            double actual_call = Analytical.vega('c', S, K, t, r, sigma) / 0.01;
            assertEquals("Error at " + i, expected_call, actual_call, EPSILON);

            double expected_put = testData.get("PV").get(i);
            double actual_put = Analytical.vega('p', S, K, t, r, sigma) / 0.01;
            assertEquals("Error at " + i, expected_put, actual_put, EPSILON);
        }
    }
    ...
    public void test_analytical_rho() {
        for (int i = 0; i < SIZE; i++) {
            ...
            double actual_call = Analytical.rho('c', S, K, t, r, sigma) / 0.01;
            assertEquals("Error at " + i, expected_call, actual_call, EPSILON);

            double expected_put = testData.get("PR").get(i);
            double actual_put = Analytical.rho('p', S, K, t, r, sigma) / 0.01;
            assertEquals("Error at " + i, expected_put, actual_put, EPSILON);
        }
    }
    ...
    public void test_numerical_delta() {
        for (int i = 0; i < SIZE; i++) {
            ...
            double expected_call = testData.get("CD").get(i);
            double actual_call = Numerical.delta('c', S, K, t, r, sigma);
            assertEquals("Error at " + i, expected_call, actual_call, EPSILON);

            double expected_put = testData.get("PD").get(i);
            double actual_put = Numerical.delta('p', S, K, t, r, sigma);
            assertEquals("Error at " + i, expected_put, actual_put, EPSILON);
        }
    }
    ...
    public void test_numerical_gamma() {
        for (int i = 0; i < SIZE; i++) {
            ...
            double expected_call = testData.get("CG").get(i);
            double actual_call = Numerical.gamma('c', S, K, t, r, sigma);
            assertEquals("Error at " + i, expected_call, actual_call, EPSILON);

            double expected_put = testData.get("PG").get(i);
            double actual_put = Numerical.gamma('p', S, K, t, r, sigma);
            assertEquals("Error at " + i, expected_put, actual_put, EPSILON);
        }
    }
    ...
    public void test_implied_volatility() {
        for (int i = 0; i < SIZE; i++) {
            ...
            double call = BlackScholes.black_scholes('c', S, K, t, r, sigma);
            double put = BlackScholes.black_scholes('p', S, K, t, r, sigma);

            try {
                double iv_call = ImpliedVolatility.implied_volatility(call, S, K, t, r, 'c');
            ...
            try {
                double iv_put = ImpliedVolatility.implied_volatility(put, S, K, t, r, 'p');
                if (iv_put != 1.0E-12) {
